...
- **CVV Recapture Support**
  ...
  - Use with `createCvvInput()` method for custom CVV collection

- **New PrimerJS Methods**
  - Added `createCvvInput()` - Creates CVV input component for custom vault UI
  - Added `startVaultPayment()` - Initiates payment with selected vaulted payment method
  - Both methods enable complete programmatic control of vault payment flows
...
checkout.options = {
  vault: {
    ...
    showEmptyState: false, // Default changed to false
  },
};
...
checkout.addEventListener('primer:ready', (event) => {
  ...
  primerJS.onVaultedMethodsUpdate = async ({
    ...
    renderCustomVaultUI(methods);

    // Show CVV input if required
    if (cvvRecapture) {
      ...
      if (selectedMethod) {
        ...
        if (cvvInput) {
          document.getElementById('cvv-container').appendChild(cvvInput);
        }
      }
    }
  };
...
- Custom vault UI design and layout
- Branded vault payment experiences
...
- [Events & Callbacks Reference](/sdk-reference/events-callbacks) - onVaultedMethodsUpdate with cvvRecapture flag
- [PrimerJS Methods Reference](/sdk-reference/events-callbacks) - createCvvInput() and startVaultPayment() methods
