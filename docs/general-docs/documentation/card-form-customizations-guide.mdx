...
You can trigger card form submission programmatically by dispatching a `primer:card-submit` event to the card form component:

```javascript
// Select the card form component
const cardForm = document.querySelector('primer-card-form');
...
:::important Component Must Be Present
Programmatic submission only works when the `primer-card-form` component is present in the UI. This approach is useful for:
