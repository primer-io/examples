...
BLIK requires SDK Core enabled (`sdkCore: true` or omit) AND must be added to `enabledPaymentMethods` array.
:::
...
```javascript
const checkout = PrimerCheckout({
  clientToken: 'token',
  sdkCore: true, // Required for BLIK
...
const checkout = PrimerCheckout({
  clientToken: 'token',
  // sdkCore not specified - used Legacy SDK
});
