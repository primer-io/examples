...
- Renamed all DOM events to follow the `primer:*` namespace format:
...
- Removed `primer-oncheckout-complete` and `primer-oncheckout-failure` events
- Replaced direct access to the headless SDK instance with a new PrimerJS class
- Consolidated payment success and failure callbacks into a single `onPaymentComplete` callback

### New Features

- Introduced the PrimerJS class with type-safe API surface:
...
- Added [migration guide](documentation/migration-guides/v01-to-v02) for transitioning from v0.1.x to v0.2.x
- Updated examples and API reference documentation
