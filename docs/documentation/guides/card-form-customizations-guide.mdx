...
:::tip New in v0.7.1
You can programmatically set the cardholder name field value using the PrimerJS instance.
:::

Beyond visual customization, you can programmatically control the cardholder name field value. This is useful for pre-filling data from user profiles or synchronizing with external forms:
...
**Important:** The `setCardholderName()` method must be called after the `primer:ready` event fires and after card inputs have been rendered. See the [Events Guide](/guides/events-guide#setting-cardholder-name-programmatically) for more examples and use cases.
